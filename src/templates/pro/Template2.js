    ...
    return (
        <div className="flex pt-2">
            <div className="float-left">
                {(contact.phone).trim() !== '' && <span className="pt-2 text-xs block"><i className="fa fa-phone"></i>{' '}{contact.phone}</span>}
                    ...
                    {(skills[skill].trim()).length > 1 &&
                        <div className={`${(Object.keys(skills).lastIndexOf(skill) !== Object.keys(skills).length - 1) ? 'border-dashed border-gray-600 border-b pb-4' : ''} pt-2  capitalize`}>
                            ...
                            {skills[skill].split(',').map(k => (
                                <span key={k} className="mr-1 mb-1 text-sm font-semibold w-auto bg-gray-600 px-2 text-white rounded inline-block">{k}</span>
                            ))}
                        </div>}
                </span>
            ))}
                            ...
                            <div className="pl-5">
                                <ul className="list-disc">
                                    {((projects[key1]['team_size']).trim() !== '' || (projects[key1]['role']).trim() !== '') &&
                                        ...
                                        <li><div className="text-sm ">{(projects[key1]['tech_stack']).trim() !== '' &&
                                        <span>
...
class Template2 extends React.Component {
    render() {
        ...
        const { bgcolor, fontColor } = this.props.proResume.templates.colors.Template2
        return (
            <React.Fragment>
